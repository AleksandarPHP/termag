...
<div class="row">
    <div class="col-md-12">
        <form method="post" action="@if(!$editing) {{ url('cms/blogs') }} @else {{ url('cms/blogs/'.$item->id) }} @endif" enctype="multipart/form-data">
            @csrf
                ...
                <div class="col-md-3">
                    ...
                    <div class="input-file-container" {!! $errors->has('image') ? 'style="border-color:red;"' : '' !!}>
                        @if(!is_null($item->image))
                            <a href="{{ url('cms/blogs/imagedelete/'.$item->id.'?image=image') }}"><span><i class="fa fa-close"></i></span></a>
                        @endif
